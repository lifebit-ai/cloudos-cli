...
The script dynamically detects column positions from the CSV header generated by `cloudos job list`. This ensures compatibility even when columns are missing from the API response.

**Required columns**:
- `ID` - Job identifier (used for deletion operations)
- `Name` - Job name (displayed in output)
- `Status` - Job status (displayed in output)

**Typical column order** (when all columns are present):
1. Status
2. Name
3. Project
4. Owner
...
> [!NOTE]
> The script does not assume fixed column positions. It parses the CSV header to find the correct positions for ID, Name, and Status columns. This prevents issues when columns like "Pipeline" are missing from the API response, which would otherwise cause column positions to shift.
