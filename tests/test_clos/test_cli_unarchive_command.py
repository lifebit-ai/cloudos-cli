...
def test_job_unarchive_command_exists():
    ...
    assert 'Unarchive specified jobs in a Lifebit Platform workspace.' in result.output
...
def test_job_unarchive_invalid_job_ids():
    ...
    with requests_mock.Mocker() as m:
        ...
        m.get(
            "https://cloudos.lifebit.ai/api/v2/jobsNew?teamId=test_workspace&archived.status=true&page=1&limit=1&id=invalid_job",
            status_code=200,
            json={"jobs": [], "pagination_metadata": {"Pagination-Count": 0}}
        )
        m.get(
            "https://cloudos.lifebit.ai/api/v2/jobsNew?teamId=test_workspace&archived.status=false&page=1&limit=1&id=invalid_job",
            status_code=200,
            json={"jobs": [], "pagination_metadata": {"Pagination-Count": 0}}
        )
