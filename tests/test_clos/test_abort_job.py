...
def test_abort_job_correct_response():
    ...
    responses.add(
            responses.PUT,
            url=f"{CLOUDOS_URL}/api/v2/jobsNew/{JOB_ID}/abort?forceAbort=false&teamId={WORKSPACE_ID}",
            headers=header,
            status=200)
...
def test_abort_job_with_force_correct_response():
    ...
    responses.add(
            responses.PUT,
            url=f"{CLOUDOS_URL}/api/v2/jobsNew/{JOB_ID}/abort?forceAbort=true&teamId={WORKSPACE_ID}",
            headers=header,
            status=200)
    # start cloudOS service
    clos = Cloudos(apikey=APIKEY, cromwell_token=None, cloudos_url=CLOUDOS_URL)
