...
output_df = pd.read_csv("tests/test_data/output_df_of_results.csv", index_col=0)
output_df_full = pd.read_csv("tests/test_data/output_df_of_results_FULL.csv", index_col=0)
...
def test_df_values_equal():
    ...
    columns_to_compare = ['_id', 'team', 'name', 'status', 'startTime', 
    'endTime', 'createdAt', 'updatedAt', 'computeCostSpent',  
    'resumeWorkDir', 'user.id', 'workflow._id', 'workflow.name', 
    'workflow.createdAt', 'workflow.updatedAt', 'workflow.workflowType', 'project._id', 
    ...
    for i in columns_to_compare:
        df_temp_list = df[i].to_list()
        real_temp_list = output_df[i].to_list()
        df_list.append(df_temp_list)
        real_list.append(real_temp_list)
    assert len(df_list) > 0 and df_list == real_list

def test_full_has_correct_columns():
    r = mocked_requests_get()
...
def test_empty_json():
    ...
    empty_json = {"a" : {"b" : ""}}
    with requests_mock.Mocker() as mock:
        mock.get(f"http://test_cloud_os/api/v1/jobs?teamId={test_workspace_id}", json= empty_json)
        r = requests.get(f"http://test_cloud_os/api/v1/jobs?teamId={test_workspace_id}")
