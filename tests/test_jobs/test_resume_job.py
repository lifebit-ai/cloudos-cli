...
@mock.patch('cloudos_cli.clos', mock.MagicMock())
@responses.activate
def test_resume_job_get_work_dir_error():
    """
    Test resume_job when getting resume work directory fails
    """
    # Load test data
    job_payload = load_json_file(JOB_PAYLOAD_INPUT)
    projects_data = load_json_file(PROJECTS_INPUT)
    ...
    # Mock GET request for job details (resume work dir) - return error
    error_message = {"statusCode": 404, "code": "NotFound",
                     "message": "Job details not found", "time": "2025-04-25_17:31:07"}
    error_json = json.dumps(error_message)
