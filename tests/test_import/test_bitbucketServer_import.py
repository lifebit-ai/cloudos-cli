...
WF_MAIN_URL = "https://bitbucketserver.com/projects/lifebit-ai/repos/post-gwas-target-identification/browse"
...
def test_fetch_correct_repo_data():
    ...
    repo_owner = parsed_url.path.split("/")[2]
    ...
    get_repo_main_file_url = f"{CLOUDOS_URL}/api/v1/git/bitbucketServer/getWorkflowConfig/{repo_name}/{repo_owner_urlencode}"
    get_repo_main_file_params = dict(host=repo_host, teamId=WORKSPACE_ID)
    responses.add(
        responses.GET,
        url=f"{CLOUDOS_URL}/api/v1/git/bitbucketServer/getPublicRepo",
