...
def test_link_folders_batch_multiple_s3(capsys, link_instance_test_response, monkeypatch):
    ...
    responses.add(responses.GET, status_url, json={"fuseFileSystems": [{"_id": "123", "mountName": "folder1", "status": "mounted"}], "paginationMetadata": {"Pagination-Count": 1}}, status=200)
    responses.add(responses.GET, status_url, json={"fuseFileSystems": [{"_id": "124", "mountName": "folder2", "status": "mounted"}], "paginationMetadata": {"Pagination-Count": 1}}, status=200)
    responses.add(responses.GET, status_url, json={"fuseFileSystems": [{"_id": "125", "mountName": "folder3", "status": "mounted"}], "paginationMetadata": {"Pagination-Count": 1}}, status=200)
    ...
    link_instance_test_response.link_folders_batch([
...
@responses.activate
def test_link_folder_v2_unavailable_raises(link_instance_test_response, monkeypatch):
    """Test that a 404 from the v2 endpoint raises a clear error (no fallback)."""
    status_url = "https://lifebit.ai/api/v1/interactive-sessions/sessionABC/fuse-filesystems?teamId=team123&limit=100&page=1"
    responses.add(responses.GET, status_url, json={"fuseFileSystems": [], "paginationMetadata": {}}, status=200)

    url_v2 = "https://lifebit.ai/api/v2/interactive-sessions/sessionABC/fuse-filesystem/mount?teamId=team123"
    responses.add(responses.POST, url_v2, status=404, json={"message": "Not Found"})

    monkeypatch.setattr(link_instance_test_response, "parse_s3_path", lambda x: {
        "dataItem": {
            "type": "S3Folder",
            "data": {"name": "folder", "s3BucketName": "bucket", "s3Prefix": "path/folder/"}
        }
    })

    with pytest.raises(ValueError, match="linking API.*not available"):
        link_instance_test_response.link_folder("s3://bucket/path/folder", "sessionABC")
